...
class TextSentiment(nn.Module):
    ...
    def __init__(self, vocab_size, embed_dim, num_class):
        ...
        """
        super().__init__()
        # 实力化embedding层，sparse=True表示每次对该层求解梯度时，只更新部分参数
    ...
    def forward(self, text):
        ...
        :return:
        """
        # 获得embedding的结果embedded, embedded.shape是(m, 32)
...
def generate_batch(batch):
    """
    :param batch: [(sample1, label1), (sample2, label2), ...]
    :return:  text = tensor([sample1, sample2, ...])
              label = tensor([label1, label2, ...])
    """
...
# 查看embedding层嵌入的词向量
print(model.state_dict()['embeddig.weight'])
